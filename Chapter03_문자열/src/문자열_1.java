 ...
 *     			random()  substring(1)	-------- substring(1,2)
 *     			length()  
 ...
 *     				  알파벳은 저장시에 1byte를 차지 : ASC/Unicode
 *     				   한글은 저장시에 2byte : Unicode
 *     					=> "UTF-8"
 *     ----------------------------------------------------
 *     원형
 *     		int length()
 ...
 *			----------------------------------------
  ...
  * 	String => 저장할 수 있는 문자으 갯수는 제한이 없다
  * 		=> 줄거리 / 회사소개
  * 3) indexOf => 지정된 문자나 문자열의 인덱스번호 찾아주는 역활
  *    => 웹(MVC)
  ...
  *    								----- 타켓형 광고
  *    	  -----------------
  *    	  startsWith / endsWith - 끝나는 문자열이 같은 경우
  *    		  |          
  ...
  *    				s="JAVA"
  *    				String s=msg.toDownCase()
  *    				s="java"
  *    
  *    	10) valueOf() ==> 모든 데이터형을 문자열로 변환
  *    	중복메소드의 정의 => 메소드명이 동일
  *    	원형) String valueOf(int i)			
  *    	원형) String valueOf(char i)			
  *    	원형) String valueOf(boolean i)			
  *    	원형) String valueOf(double i)			
  *    	원형) String valueOf(float i)			
  *    	원형) String valueOf(byte i)			
  *    	원형) String valueOf(char[] i)
  *    	=> static으로 제작	=> 공통적인 
