 ...
 * => 제어어
 * 	   static : 공통적인
 * 	   final : 마지막인 => 상수 / 변경하지 않는 메소드를 만들때 사용
 * 	   abstract  : 추상적인 => 공통기반의
 ...
 * ↑
 * B => sub => this , super
 * ***  상속을 내리는 클래스 => 상속받은 클래스의 변수나 메소드의 제어가 불가능
 * 		-------------- this,super
 ...
 *  claa A extends Object 
 *  자바의 모든 라이브러리는 예외처리를 가지고 있다
 *  
 *  => 201page
 *  1) 단일 상속만 가능 => 수정 가능 (변수오버라이딩, 메소드오버라이딩)
 *  2) 상속 : extends : 한개만 사용이 가능
 *  3) 상속을 두개이상 받는 경우
 *  	class A
 *  	class B
 *  	class C exends A , B => 오류
 *  	
 *  	class A
 *  	class B
 *  	class C exends A => A, B 
 *  	class C exends B =>C, B, A
 *  
 *  	=> 여러개를 받는 방법 => 클래스는 기존
 *  		-------------- 인터페이스
 *  
