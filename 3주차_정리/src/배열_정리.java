 ...
 * 					2) int이하 => char,short,byte => 연산이 되면 결과값 int다
 * 									int + long = long
 * 									char + char = int
 * 									byte + byte = int
 * 					+ : 산술(더하기)
 ...
 *                  	  특수문자는 ($,_)
 *                  	*** 소문자로 시작한다
 *                  	*** 구분자 => 메소드명은 한개...구분자명은 중복이 없게
 *                  				약간의 의미만 부여
 *                  				7~25자 사이
 *                
 *          	매개변수 : 여러개 사용이 가능 . 없는 경우도 있다
 *          				trim(), math.random()
 *          			 => 가급적이면 3개이상이면 클래스/ㅐ열을 사용
 *          		     => 사용자 요청해서 보내주는 값
 *          			 로그인
