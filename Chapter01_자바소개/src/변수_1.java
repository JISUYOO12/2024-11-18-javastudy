/*
 	...
 	1. 프로그램에 필요한 데이터 저장
 		...
 		===> 모든 프로그램에 변수가 업슨 프로그램은 존재하지 않는다.
 				============= 은퇴
				프로그램 : 변수 /연산자/제어문
				======================= 관련된 내용을 묶어서 사용
				(입력 / 처리 / 결과 => 메소드) => 클래스
 		...
 		1) 정수 저장
 			...
 			4byte : int => 기본 : 컴퓨터는 모든 정수는 int로 인식
  			...
  			system.out.printin(1000);
  					
 			8byte : long => 금융권 / 증권 / 공기업
 			실수 저장
 			float : 4byte => 소수점 : 6자리
 			double : 8byte => 소수점 : 15자리
 					 컴퓨터에서는 디폴트(모든실수는 double로 인식)
 		3) 문자 저장
 		...
 		4) 논리(true/false) : 1byte
 			...
 			* 구분
 			 ...
 			 큰 데이터형 : 작은 값을 대입받을 수 있음
 			 작은 데이터형 : 큰 값을 대입받을 수 없음
 					ex) long에 int값 대입 가능
 					 	int에 long값 대입 x
