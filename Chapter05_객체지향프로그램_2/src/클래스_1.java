 ...
 *  	=> 보완
 *  	=> 동일한 목적이나 기능을 수해하는 변수나 메소드를 묶어서 한개의 파일을 제작 (객체)
 *  	   객체끼리 상호 통신(메소드) 전체 프로그램
 *  	  ---------------------------------
 *  	  예) 
 *  		컴퓨터
 *  		CPU / 하드디스크 / 그래픽 카드 / 메모리  => 객체
 *  		--------------------------------------
 *  		| 메인보드(조립)
 *  		사이트
 *  		회원 관련 , 예약 관련 , 구매(장바구니) , 
 ...
 *      클래스 => 객체를 사용하기 위한 설계
 *      형식)
 *      	[접근지정어][제어어]class ClaaName
 *      	{	클래스 블럭에서 선언 => 다른 클래스에서도 사용이 가능
 *      		----------------------
 *      		객체 생성시마다 따로 메모리에 저장되는 변수
 *      		----------- new 생성자() 
 *      		=> 멤버변수 : Heap => 다른 데이터형을 모아서 저장이 가능
 *      		메모리 공간을 한개만 만들어서 공유
 *      		=> 공유변수 => static
 *      		  ------- 데이터가 여러개 생성 
 *      				  영화목록 / 음악목록 / 회원관리
...
class MemberSystem
{
	static Member[] members=new Member[10]; //Member => null 공간만 할당
