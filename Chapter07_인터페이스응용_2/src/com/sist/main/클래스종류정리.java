 ...
 * 					  = 윈도우 이벤트 처리
 * 					  = Collection
 * 					  	1) 순서가 있다 , 중복 데이터 저장을 허용
 * 						   List =>ArrayList / LinkedList
 * 												|
 * 												Queue
 * 						2) 순서가 없다 , 중복 데이터 저장을 허용하지 않는다
 * 						   Set	=> TreeSet / HashSet
 * 						3) 키 , 값을 동시에 저장 , 키 중복이 없다 , 데이터는 중복이 가능하다
 ...
 * 						----------------
 * 						| 자신의 생성자를 이용해서 메모리 할당을 할 수 없다
 * 		=> 클래스가 미완성이기 때문엘 메모리 할당을 할 수 없다
 * 				------
 * 				| 메소드가 구현이 안된 상태	
 * 		=> 상속 : extends 
 * 		 => 제외 : static , 초기화블럭 / 생성자
 ...
 * 			}
 	...
 	1) 형식
 	   ...
 	   인터페이스 
 	     ...
 	     *** 서로다른 클래스 여러개 묶어서 한개의 이름으로 제언할때 주로 사용
